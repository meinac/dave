export default class WaterColumn {
  ...
  */
  static FRESH_WATER_DENSITY = 1.000;

  /*
  * Typical density of sea water in kg/L.
  */
  static SALT_WATER_DENSITY = 1.030;

  /*
  * Standard atmospheric pressure at sea level (1 atm = 1.01325 bar).
  */
  static ATMOSPHERIC_PRESSURE_AT_SEA_LEVEL = 1.01325;
  ...
  */
  static SEA_LEVEL_TEMPERATURE_K = 288.15;
  ...
  */
  static BAROMETRIC_EXPONENT = 5.255;
  ...
  */
  static METERS_OF_FRESH_WATER_PER_BAR = 10.197;

  static density(waterType) {
    return waterType === this.SALT_WATER ? this.SALT_WATER_DENSITY : this.FRESH_WATER_DENSITY;
  }

  static surfacePressure(altitude = 0) {
    const lapseRatio = (this.TEMPERATURE_LAPSE_RATE * altitude) / this.SEA_LEVEL_TEMPERATURE_K;

    return this.ATMOSPHERIC_PRESSURE_AT_SEA_LEVEL * Math.pow(1 - lapseRatio, this.BAROMETRIC_EXPONENT);
  }
  ...
  static gaugePressureAt(depth, waterType = this.SALT_WATER) {
    return (depth * this.density(waterType)) / this.METERS_OF_FRESH_WATER_PER_BAR;
  }
  ...
  static depthFromGaugePressure(pressure, waterType = this.SALT_WATER) {
    return (pressure * this.METERS_OF_FRESH_WATER_PER_BAR) / this.density(waterType);
  }
}
